...
#' \code{\link{validation_scrape_json_play_by_play}} and \code{\link{scrape_html_play_by_play}}
...
#' \code{\link{validation_scrape_season_play_by_play}} function that will perform its own
...
validation_scrape_game_play_by_play <- function(game_id, type, season, check_url = 1) {
  ...
  if (season >= 2009) {
    game_pbp <- validation_scrape_json_play_by_play(game_id, check_url)
  } else {
...
#' playoff_game_ids_17 <- validation_scrape_game_ids(2017, type = "post")
#' sb_17_id <- playoff_game_ids_17$game_id[nrow(playoff_game_ids_17)]
#' sb_17_pbp <- validation_scrape_json_play_by_play(game_id = sb_17_id)
#' @export

validation_scrape_json_play_by_play <- function(game_id, check_url = 1) {
  tictoc::tic(msg="validation_scrape_json_play_by_play")
  ...
  game_url <- nflscrapR::create_game_json_url(game_id)
  ...
  get_play_data <- function(play) {
    ...
    if (length(play$players) > 0) {
      # This is used to measure the run time of the old code and the new code
      # and determine whether or not the new code improves performance
      # tictoc doesn't have great precision, but thats ok for the moment.
      tictoc::tic(msg = "current_code")
      ...
      play_data <- purrr::map(indicator_stats,
        ...
        dplyr::mutate(rush_attempt = dplyr::if_else(any(c("rushing_yards",
                      ...
                      lateral_recovery = dplyr::if_else(any(c("lateral_own_fumble_recovery_yards",
                                                              ...
                                                              play_player_data$nfl_stat),
                                                        1, 0))
      #new function replicates functionality up to this point
      tictoc::toc(log = TRUE, quiet = TRUE)
      tictoc::tic("new_function")
      new_function_test <- new_function(play)
      tictoc::toc(log = TRUE, quiet = TRUE)
      tictoc::tic("confirmation_of_new_function")
      assertthat::assert_that(identical(new_function_test$new_play_data, play_data), msg = "proposed changes do not match original play_data")
      assertthat::assert_that(identical(new_function_test$new_player_ids, player_ids), msg = "proposed changes do not match original player_ids")
      assertthat::assert_that(identical(new_function_test$new_play_player_data, play_player_data), msg = "proposed changes do not match original play_player_data")
      assertthat::assert_that(identical(new_function_test$new_indicator_stats, indicator_stats), msg = "proposed changes do not match original indicator_stats")
      rm(new_function_test)
      tictoc::toc(log = TRUE, quiet = TRUE)
  ...
  #This is slightly changed to allow for timing validation_scrape_json_play_by_play
  # App the expected points columns:
  game_pbp <-
    nflscrapR::add_ep_variables(game_pbp) %>%
...
validation_scrape_season_play_by_play <- function(season, type = "reg", weeks = NULL, teams = NULL) {
  tictoc::tic(msg="validation_scrape_season_play_by_play")
  # Gather the game ids based on the inputs:
  game_ids <- nflscrapR::scrape_game_ids(season, type, weeks, teams) %>%
  ...
  game_ids_check <- purrr::map_lgl(game_ids,
                                   function(x) {
                                     game_url <- nflscrapR::create_game_json_url(x)
  ...
  result <- purrr::map_dfr(game_ids[game_ids_check],
                 function(x) {
                   validation_scrape_game_play_by_play(game_id = x, type, season,
                                            check_url = 0)
                 })
